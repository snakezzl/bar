...
- 🎯 Entry points to execute your program using `python -m <bar>` or `$ bar` with basic CLI argument parsing.
...
---
# bar Flask Application

Awesome bar created by snakezzl
...
```bash
git clone https://github.com/snakezzl/bar bar
cd bar
make install
```
...
```bash
bar create-db   # run once
bar populate-db  # run once (optional)
bar add-user -u admin -p 1234  # ads a user
bar run
```
